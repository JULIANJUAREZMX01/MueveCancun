...
export function initAnimations(root) {
    ...
    const elements = (root || document).querySelectorAll('[data-animate]:not([data-animated])');
    observer?.disconnect();
...
function addRipples() {
    document
        .querySelectorAll('.mc-btn:not([data-ripple]), .nav-item:not([data-ripple]), .map-ctrl-btn:not([data-ripple]), .quick-btn:not([data-ripple])')
        .forEach(el => {
